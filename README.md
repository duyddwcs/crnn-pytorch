...
Unlike a traditional deep neural network, which uses different parameters at each layer, a RNN shares the same parameters (U, V, W above) across all steps. Beside being able to efficiently process sequences of varying length, sharing parameters reflects the fact that we are performing the same task at each step, just with different inputs. This greatly reduces the total number of parameters we need to learn.
...
where B(π) represents the set of all paths of text l after transformation from sequence to sequence mapping function B, and π is one of the paths. The probability of π is defined as <img src="https://render.githubusercontent.com/render/math?math=p(\pi |y) = \prod_{t=1}^{T} y_{\pi t}^{t}">, <img src="https://render.githubusercontent.com/render/math?math=y_{\pi t}^{t}"> is the probability of having label πt at time step t.
